clear; clc; close all;
% removing columns corresponding to circle centers
% which image
row = 1;
col = 1;
...
if(spacing == 0)
    ...
    px = [mid - total_spacing, mid + total_spacing];
    py = [mid, mid];
    remove_x = [mid - total_spacing, mid, mid + total_spacing];
    remove_y = [1, mid, N_start];
end
...
% remove rows
obj = remove_elts(obj, remove_x, remove_y);
...
hold on;
imagesc(obj_amp);
colormap(amp_colormap(200, 140, 180));
colorbar;
axis square;
axis tight;
axis off;
